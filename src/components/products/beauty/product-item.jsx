    ...
    >
      <div className="tp-product-thumb-3 mb-15 fix p-relative z-index-1">
        <Link href={`/product-details/${_id}`}>
          <Image src={default_item?.img} alt="product image" width={282} height={320} />
        </Link>

        <div className="tp-product-badge">
          {status === 'out-of-stock' && <span className="product-hot">{t("product.badge.outOfStock")}</span>}
        </div>
        ...
        <div className="tp-product-action-3 tp-product-action-blackStyle">
          <div className="tp-product-action-item-3 d-flex flex-column">
            {isAddedToCart ? (
              ...
              >
                <Cart />
                <span className="tp-product-tooltip">{t("product.actions.viewCart")}</span>
              </Link>
            ) : (
              <button
                type="button"
